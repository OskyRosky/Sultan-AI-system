...
DebateSummary
  ...
  debate_generation_mode
  prompt_template_id
  prompt_version
  llm_model_metadata_ref
  bull_arguments
  bear_arguments
...
`debate_generation_mode`, `prompt_template_id`, `prompt_version`, `llm_model_metadata_ref`, `validation_status`, and `fallback_status` are audit and replay metadata only. They are not evidence, confidence, approval, or downstream eligibility.
...
## 14. debate_balance_status Enum

Allowed `debate_balance_status` values are:
...
unavailable
rejected_due_to_contract_violation
```

`debate_balance_status` is argument balance metadata only.

`debate_balance_status` is not confidence.

`debate_balance_status` is not trading confidence.

`debate_balance_status` is not trade approval.

`debate_balance_status` is not Paper Trading readiness.

`debate_balance_status` is not Live Trading readiness.

`debate_balance_status` must not override `missing_evidence`, `blocking_gaps`, `forbidden_downstream_usage`, or Motor B `framework_only`.

Assignment rules:

| Value | Deterministic assignment rule |
| --- | --- |
| `bull_favorable` | Bull arguments are stronger within the normalized input boundaries, with all missing evidence, limitations, and restrictions preserved. This must not imply confidence increase. |
| `bear_favorable` | Bear arguments are stronger within the normalized input boundaries. This must not imply automatic veto or Risk Engine decision. |
| `balanced` | Bull and Bear arguments are materially balanced. This must not imply safe-to-trade, approval, or readiness. |
| `unresolved` | Material uncertainty, conflict, missing evidence, source contradiction, or disagreement remains unresolved. The uncertainty must be preserved. |
| `insufficient_arguments` | The debate lacks sufficient bounded arguments, references, or role coverage. The output must be degraded or routed to human review depending on materiality. |
| `unavailable` | Debate balance cannot be produced because debate metadata is unavailable. `non_approval_statement` must remain explicit. |
| `rejected_due_to_contract_violation` | DebateSummary cannot be trusted because of schema, source, prompt, raw input, prohibited output, or contract violations. |

Block 08 may consume `debate_balance_status` only as metadata, not as confidence, empirical evidence, fusion authorization, Paper Trading readiness, Live Trading readiness, or trade approval.

## 15. prohibited_inference_flags Enum

`prohibited_inference_flags` captures auditable detections of prohibited language, inference, or output behavior generated by Bull/Bear debate, especially when debate generation is LLM-assisted.

Allowed `prohibited_inference_flags` values are:

```text
no_prohibited_inference_detected
...
capital_allocation_language_detected
strategy_promotion_language_detected
confidence_invention_detected
empirical_evidence_invention_detected
unsupported_claim_promoted_detected
missing_evidence_ignored_detected
blocking_gap_ignored_detected
forbidden_downstream_usage_ignored_detected
motor_b_framework_only_override_attempted
raw_input_usage_detected
prompt_injection_suspected
source_fabrication_detected
unsupported_market_prediction_detected
risk_engine_bypass_language_detected
unknown_prohibited_inference_detected
```

`prohibited_inference_flags` must be populated when debate output violates or attempts to bypass Block 05, Block 06, Block 07, Block 08, Block 09, or `08 Risk Engine` boundaries.

These flags are validation metadata.

`prohibited_inference_flags` are validation metadata.

These flags are not confidence scores.

These flags are not evidence.

These flags are not Risk Engine decisions.

Functional rules:

- if no prohibited inference is detected, use `no_prohibited_inference_detected`;
- if trade approval language is detected, `DebateSummary` must be rejected;
- if execution language is detected, `DebateSummary` must be rejected;
- if raw input usage is detected, `DebateSummary` must be rejected;
- if confidence invention is detected, `DebateSummary` must be rejected or degraded depending on severity;
- if empirical evidence invention is detected, `DebateSummary` must be rejected;
- if `forbidden_downstream_usage` or Motor B `framework_only` is ignored or overridden, `DebateSummary` must be rejected or require human review;
- if prompt injection is suspected, `DebateSummary` must be degraded, rejected, or require human review depending on severity;
- if source fabrication is detected, `DebateSummary` must be rejected;
- if risk engine bypass language is detected, `DebateSummary` must be rejected.

`prohibited_inference_flags` must be referenced by Debate validation rules, Human review triggers, LLM safety boundaries, Prompt contract requirements, and Audit and replay metadata.
...
## 17. Motor B framework_only Rule
...
## 20. Unsupported Claims Handling
...
## 21. Conflicting Sources Handling
...
## 22. Forbidden Downstream Usage Propagation
...
## 23. Synthetic And Dry-Run Handling
...
## 24. LLM Safety Boundaries For Debate Generation
...
Prohibited inference detections must be preserved in `prohibited_inference_flags`.
...
## 25. Prompt Contract Requirements For Debate
...
- required `debate_balance_status`;
- required `prohibited_inference_flags`;
- fallback behavior for prohibited inference detection.

## 26. Debate Validation Rules
...
- output includes `debate_balance_status`;
- output includes `prohibited_inference_flags`;
...
- has `prohibited_inference_flags` containing `trade_approval_language_detected`;
- has `prohibited_inference_flags` containing `execution_language_detected`;
- has `prohibited_inference_flags` containing `raw_input_usage_detected`;
- has `prohibited_inference_flags` containing `source_fabrication_detected`;
- has `prohibited_inference_flags` containing `risk_engine_bypass_language_detected`.
...
If `debate_balance_status = rejected_due_to_contract_violation`, validation status must be rejected.

If `debate_balance_status = insufficient_arguments`, validation status must be degraded or require human review depending on missing evidence, event severity, and downstream materiality.

## 27. Human Review Triggers
...
- `prohibited_inference_flags` contains any severe prohibited inference;
- `debate_balance_status = unresolved`;
- `debate_balance_status = insufficient_arguments` with material downstream impact;
...
## 28. Audit And Replay Metadata Requirements
...
- which rule assigned `debate_balance_status`;
- which validations populated `prohibited_inference_flags`;
- which restrictions were preserved;
- why the debate output was accepted, degraded, or rejected.

## 29. Relationship With 07-Block-08 Deterministic Signal Fusion Engine
...
Block 08 may consume `debate_balance_status` and `prohibited_inference_flags` only as validation and argument metadata.

Block 08 must not treat `debate_balance_status` or `prohibited_inference_flags` as confidence, evidence, eligibility, or fusion authorization.

## 30. Relationship With 07-Block-09 Confidence Status And Aggregation Policy
...
`debate_balance_status` is not confidence and must not be mapped into final signal confidence.

`prohibited_inference_flags` are validation metadata and must not be mapped into final signal confidence.
...
- convert `prohibited_inference_flags` into confidence;
...
- debate output contract;
- `DebateSummary` schema;
- audit/replay metadata fields for debate generation mode, prompt template, prompt version, LLM model metadata reference, validation status, and fallback status;
- Bull argument requirements;
- Bear argument requirements;
- uncertainty and disagreement labels;
- `debate_balance_status` enum;
- `prohibited_inference_flags` enum and functional handling;
- missing evidence handling;
- blocking gaps handling;
- unsupported claims handling;
- conflicting sources handling;
